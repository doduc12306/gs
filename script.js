...
// ===== HÀM ĐỌC VÀ PARSE CSV =====

/**
 * Parse một dòng CSV, xử lý đúng trường có dấu ngoặc kép và chứa dấu phẩy
 */
...
/**
 * Parse toàn bộ nội dung CSV thành mảng session objects
 */
function parseCSV(csvText) {
    const lines = csvText.split(/\r?\n/).filter(l => l.trim() !== '');
    if (lines.length < 2) return []; // chỉ có header hoặc rỗng
    ...
    return lines.slice(1).map(line => {
        ...
        // Parse các trường đặc biệt
        let homework = [];
        try {
            homework = JSON.parse(obj.homework || '[]');
        } catch (e) {
            homework = [];
        }
        ...
        return {
            ...
            homework: homework,
            comment: obj.comment || '',
            reviewNeeded: obj.reviewNeeded || '',
            understandLevel: parseInt(obj.understandLevel) || 0
        };
    });
}

/**
 * Lấy danh sách buổi học từ file CSV
 */
async function fetchSessions() {
    try {
        ...
        cachedSessions = parseCSV(csvText);
        return cachedSessions;
    } catch (error) {
        console.error('Lỗi đọc CSV:', error);
        showToast('❌ Không thể tải file dữ liệu CSV');
        return [];
    }
}
